...
<h2>아쉬운점</h2>
 - 데이터를 검색할때마다 API를 통해 받아오는것이 아닌 데이터베이스에 저장해서 검색했으면, 더 많은 검색조건을 만들 수 있었을것이며 속도도 향상됬을것임<br>
 - 네이버 지도 API에 길찾기 내용을 지도위에 출력하는 기능이 없는점이 아쉬움<br>
 - 휴진이 아닌곳만 출력되도록 하고 싶었지만, 병원별로 데이터가 '휴진','전부휴진' 등 통일화가 되있지 않아서 미구현<br>
